    ...
    public class HomeControllerTests
    {
        ...
        public async Task GetTest()
        {
            ...
            mockHttpMessageHandler.Protected().Setup<Task<HttpResponseMessage>>("SendAsync", ItExpr.IsAny<HttpRequestMessage>(), ItExpr.IsAny<CancellationToken>())
                ...
                { 
                    StatusCode = HttpStatusCode.OK, Content = new StringContent(successContent), }
                ); 
            var client = new HttpClient(mockHttpMessageHandler.Object); 
            mockFactory.Setup(_ => _.CreateClient(It.IsAny<string>())).Returns(client);

            var testController = new HomeController(new NullLogger<HomeController>(), mockFactory.Object);
